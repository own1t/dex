...
contract("Dex", (accounts) => {
  ...
  it("should NOT deposit any tokens if token does not exist", async () => {
    await expectRevert(
      dex.deposit(
  ...
  it("should NOT withdraw any tokens if token does not exist", async () => {
    await expectRevert(
      dex.withdraw(
        web3.utils.toWei("100"),
        web3.utils.fromAscii("TOKEN-DOES-NOT-EXIST"),
  ...
  it("should NOT withdraw any tokens if there is not enough balance", async () => {
  ...
  it("should NOT create limit order if there is not enough balance", async () => {
  ...
  it("should NOT create limit order if there is not enough balance for DAI", async () => {
    await dex.deposit(web3.utils.toWei("99"), REP, { from: trader1 });
  ...
  it("should NOT create limit order if the token is DAI", async () => {
    await expectRevert(
  ...
  it("should NOT create limit order if the token does not exist", async () => {
    await expectRevert(
  ...
  it("should create market order & match with existing limit order", async () => {
    await dex.deposit(web3.utils.toWei("100"), DAI, { from: trader1 });
  ...
  it("should NOT create market order if there is not enough balance", async () => {
    await expectRevert(
      dex.createMarketOrder(REP, web3.utils.toWei("1"), SIDE.SELL, {
        from: trader2,
      }),
      "Not enough balance"
    );
  ...
  it("should NOT create market order if there is not enough balance for DAI", async () => {
